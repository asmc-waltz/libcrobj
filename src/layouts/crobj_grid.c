...
static int32_t set_dsc_data(grid_desc_t *dsc, int32_t val)
{
    int32_t ret;
                                   ...
                                   int32_t value, dsc_op_t type)
{
    ...
    ret = set_dsc_data(next_dsc, value);
    if (ret) {
    ...
    for (i = 0; i < dsc->size; i++) {
        ret = set_dsc_data(next_dsc, dsc->cell_pct[i]);
        if (ret)
            goto out_free;
    }
...
int32_t rotate_grid_dsc_90(lv_obj_t *lobj)
{
    ...
    for (i = 0; i < c_dsc->size; i++) {
        ret = set_dsc_data(next_r_dsc, c_dsc->cell_pct[i]);
        if (ret)
            goto out_free_c;
    }

    for (i = (r_dsc->size - 1); i >= 0; i--) {
        ret = set_dsc_data(next_c_dsc, r_dsc->cell_pct[i]);
        if (ret)
            goto out_free_c;
    }
